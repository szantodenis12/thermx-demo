...
vec3 render(vec2 st, vec2 mouse) {
    ...
    // Subtle dark gray to blend with background
    vec3 color = vec3(0.2, 0.2, 0.2);
    
    // Subtle dimming on hover
    float dimming = 1.0 - mouseInfluence * 0.1;
...
export default function GeometricBlurMesh({ className = "" }: { className?: string }) {
    const containerRef = useRef<HTMLDivElement>(null);
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const mouseRef = useRef({ x: 0, y: 0 });
    const mouseDampRef = useRef({ x: 0, y: 0 });
    const [currentShape, setCurrentShape] = useState(4); // Pyramid init
    ...
    useEffect(() => {
        ...
        // Transparent clear color so it acts like a background
    ...
    useEffect(() => {
        ...
        const animate = (time: number) => {
            ...
            // Clear and render
            // Transparent clear color here too
            gl.clearColor(0.0, 0.0, 0.0, 0.0);
    ...
    return (
        <div ref={containerRef} className={`absolute inset-0 pointer-events-auto opacity-70 z-0 ${className}`}>
            <canvas 
                ref={canvasRef}
